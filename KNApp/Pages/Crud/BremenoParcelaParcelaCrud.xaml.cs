...
public sealed partial class BremenoParcelaParcelaCrud
{
    ...
    {
        var data = await LoadDataAsync("/bremeno_parcela_parcela",
            AppJsonContext.Default.BremenoParcelaParcelaDataList);
        if (data != null)
        {
            Data = data;
        }
    ...
    private async void CreateButtonClick(object sender, RoutedEventArgs e)
    {
        if (await CreateItemAsync("/bremeno_parcela_parcela", NewItem,
                AppJsonContext.Default.BremenoParcelaParcelaData))
        {
        ...
        if (sender is Button button && button.Tag is BremenoParcelaParcelaData item)
        {
            try
            {
                var response = await HttpService.DeleteData(
                    $"/bremeno_parcela_parcela?parcela_id={item.ParcelaId}&parcela_povinna_id={item.ParcelaPovinnaId}");
